...
export function SettingsScreen({ model, connection, machineConnection, scale, onClose }: SettingsScreenProps) {
  ...
  const overview = <div className="settings-overview">
    ...
    <SettingsCard eyebrow="Bestpresso" title="Feedback & charts" description="Preferences stored for this skin"><div className="settings-list"><div className="settings-row"><span>Completion sound</span><strong>{preferences.completionSoundEnabled ? 'On' : 'Off'}</strong></div><div className="settings-row"><span>Chart lines</span><strong>{{ fine: 'Thin', standard: 'Medium', bold: 'Thick' }[preferences.chartLineWeight]}</strong></div><div className="settings-row"><span>Water warning</span><strong>{preferences.waterWarningLevelMl} ml</strong></div></div><button className="settings-card-link" type="button" onClick={() => setActiveSection('experience')}>Personalize Bestpresso <span>→</span></button></SettingsCard>
  ...
  const power = <div className="settings-grid"><div className="settings-stack"><SettingsCard eyebrow="Display" title="Screen brightness"><SectionList><NumberSetting label="Brightness" hint={settings.draft.display.lowBatteryBrightnessActive ? 'Currently capped by low battery' : '100 returns control to the operating system'} value={numberValue(settings.draft.display.requestedBrightness ?? settings.draft.display.brightness)} unit="%" {...SETTINGS_PROTOCOL.brightness} disabled={settings.unavailable.includes('display') || settings.draft.display.platformSupported?.brightness === false} onChange={(requestedBrightness) => settings.patchDisplay({ requestedBrightness })} /><SwitchSetting label="Limit brightness on low battery" checked={settings.draft.rea.lowBatteryBrightnessLimit} disabled={reaUnavailable} onChange={(lowBatteryBrightnessLimit) => settings.patchRea({ lowBatteryBrightnessLimit })} /><NumberSetting label="Screensaver brightness" hint="Used while the machine sleeps. Default: 7%." value={preferences.screensaverBrightness} unit="%" min={0} max={100} step={1} onChange={(screensaverBrightness) => updatePreferences({ screensaverBrightness })} /></SectionList></SettingsCard><SettingsCard eyebrow="Battery" title="Charging"><SectionList><SelectSetting label="Charging mode" value={settings.draft.rea.chargingMode} disabled={reaUnavailable} options={[{ value: 'disabled', label: 'Disabled' }, { value: 'longevity', label: 'Battery longevity' }, { value: 'balanced', label: 'Balanced' }, { value: 'highAvailability', label: 'High availability' }]} onChange={(chargingMode) => settings.patchRea({ chargingMode: chargingMode as 'disabled' | 'longevity' | 'balanced' | 'highAvailability' })} /><SwitchSetting label="Overnight charging schedule" checked={settings.draft.rea.nightModeEnabled} disabled={reaUnavailable} onChange={(nightModeEnabled) => settings.patchRea({ nightModeEnabled })} /><div className="settings-time-pair"><label><span>Sleep time</span><input type="time" disabled={reaUnavailable} value={timeText(settings.draft.rea.nightModeSleepTime)} onChange={(event) => settings.patchRea({ nightModeSleepTime: timeMinutes(event.target.value) })} /></label><label><span>Morning time</span><input type="time" disabled={reaUnavailable} value={timeText(settings.draft.rea.nightModeMorningTime)} onChange={(event) => settings.patchRea({ nightModeMorningTime: timeMinutes(event.target.value) })} /></label></div></SectionList></SettingsCard></div><div className="settings-stack"><SettingsCard eyebrow="Sleep & wake" title="Sleep & presence"><SectionList><SwitchSetting label="Keep screen awake" checked={settings.draft.rea.keepAwake} disabled={reaUnavailable} onChange={(keepAwake) => settings.patchRea({ keepAwake })} /><SwitchSetting label="Presence detection" checked={settings.draft.presence.userPresenceEnabled} disabled={settings.unavailable.includes('presence')} onChange={(userPresenceEnabled) => settings.patchPresence({ userPresenceEnabled })} /><NumberSetting label="Sleep after" hint="0 disables automatic idle sleep" value={numberValue(settings.draft.presence.sleepTimeoutMinutes)} unit="min" {...SETTINGS_PROTOCOL.presenceTimeout} disabled={settings.unavailable.includes('presence')} onChange={(sleepTimeoutMinutes) => settings.patchPresence({ sleepTimeoutMinutes })} /></SectionList></SettingsCard><SettingsCard eyebrow="Schedule" title="Wake times" description="Each schedule is an independent Decaid record and saves immediately." action={<button className="settings-reset" type="button" disabled={settings.unavailable.includes('presence')} onClick={() => void settings.createSchedule({ time: '07:00', daysOfWeek: [1, 2, 3, 4, 5], enabled: true, keepAwakeFor: 60 })}>Add schedule</button>}><div className="settings-schedule-list">{(settings.draft.presence.schedules || []).map((schedule) => <div className="settings-schedule" key={schedule.id}><input aria-label="Wake time" type="time" value={schedule.time || timeText(schedule.hour === undefined ? undefined : schedule.hour * 60 + (schedule.minute || 0))} onChange={(event) => schedule.id && void settings.updateSchedule(schedule.id, { time: event.target.value })} /><span>{schedule.daysOfWeek?.length ? schedule.daysOfWeek.length === 7 ? 'Every day' : `${schedule.daysOfWeek.length} days` : 'Every day'}</span><Toggle label="Schedule enabled" checked={schedule.enabled !== false} onChange={(enabled) => schedule.id && void settings.updateSchedule(schedule.id, { enabled })} /><button type="button" className="settings-reset" onClick={() => schedule.id && window.confirm('Delete this wake schedule?') && void settings.deleteSchedule(schedule.id)}>Remove</button></div>)}{!(settings.draft.presence.schedules || []).length && <p className="settings-empty">No wake schedules configured.</p>}</div></SettingsCard></div></div>

  const experience = <div className="settings-grid"><SettingsCard eyebrow="Sound & feedback" title="Play sound on shot complete" wide action={<Toggle checked={preferences.completionSoundEnabled} onChange={(completionSoundEnabled) => updatePreferences({ completionSoundEnabled })} label="Play sound on shot complete" />}><></></SettingsCard><SettingsCard eyebrow="Appearance" title="Display"><SectionList><SelectSetting label="Bestpresso theme" hint="Light mode is in beta. Dark is the default." value={preferences.theme} options={[{ value: 'dark', label: 'Dark' }, { value: 'light', label: 'Light (Beta)' }]} onChange={(value) => updatePreferences({ theme: value === 'light' ? 'light' : 'dark' })} /><SelectSetting label="Decaid system theme" hint="Appearance outside Bestpresso" value={settings.draft.rea.themeMode} disabled={reaUnavailable} options={[{ value: 'system', label: 'Follow device' }, { value: 'light', label: 'Light' }, { value: 'dark', label: 'Dark' }]} onChange={(themeMode) => settings.patchRea({ themeMode: themeMode as 'system' | 'light' | 'dark' })} /><SelectSetting label="Clock format" hint="Screensaver and history times in Bestpresso" value={preferences.clockFormat} options={[{ value: 'device', label: 'Follow device' }, { value: '12h', label: '12-hour (AM/PM)' }, { value: '24h', label: '24-hour' }]} onChange={(value) => updatePreferences({ clockFormat: value === '24h' ? '24h' : value === '12h' ? '12h' : 'device' })} /><SelectSetting label="Temperature" hint="Bestpresso display only; Decaid stays in Celsius" value={temperatureUnit} options={[{ value: 'C', label: 'Celsius (°C)' }, { value: 'F', label: 'Fahrenheit (°F)' }]} onChange={(value) => updatePreferences({ temperatureUnit: value === 'F' ? 'F' : 'C' })} /></SectionList></SettingsCard><div className="settings-stack"><SettingsCard eyebrow="Charts & monitoring" title="Line weight" description="Apply one telemetry line weight throughout Bestpresso."><div className="settings-line-options">{(['fine', 'standard', 'bold'] as ChartLineWeight[]).map((weight) => <button className={preferences.chartLineWeight === weight ? 'is-selected' : ''} type="button" key={weight} onClick={() => updatePreferences({ chartLineWeight: weight })}><svg viewBox="0 0 120 34" aria-hidden="true"><path d="M2 27 C27 27 27 8 54 8 S82 25 118 13" /></svg><span>{{ fine: 'Thin', standard: 'Medium', bold: 'Thick' }[weight]}</span></button>)}</div></SettingsCard><SettingsCard eyebrow="Bestpresso" title="Reset preferences" description="Restore this skin’s defaults. Your profiles and machine settings stay unchanged."><button className="settings-reset settings-reset--all" type="button" onClick={resetPreferences}>Reset Bestpresso preferences</button></SettingsCard></div></div>
