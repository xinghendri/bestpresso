...
export function SettingsScreen({ model, connection, machineConnection, scale, onClose }: SettingsScreenProps) {
  ...
  const power = <div className="settings-grid">
    <SettingsCard eyebrow="Bestpresso" title="Screensaver"><SectionList><NumberSetting label="Brightness" hint="Used while the machine sleeps. Default: 7%." value={preferences.screensaverBrightness} unit="%" min={0} max={100} step={1} onChange={(screensaverBrightness) => updatePreferences({ screensaverBrightness })} /></SectionList></SettingsCard>
