...
const editForMetric = (utility: MachineUtility, label: string, onSave?: (setting: EditableMachineSetting, value: number) => void, disabled?: boolean) => {
  ...
  if (utility.id === 'water' && label === 'Target yield') return { min: 1, max: 500, step: 1, disabled, onSave: (value: number) => onSave('hotWaterVolume', value) }
  if (utility.id === 'water' && label === 'Temp.') return { min: 40, max: 100, step: 1, disabled, onSave: (value: number) => onSave('hotWaterTemperature', value) }
  if (utility.id === 'steam' && label === 'Target') return { min: 135, max: 165, step: 1, disabled, onSave: (value: number) => onSave('steamTemperature', value) }
  if (utility.id === 'steam' && label === 'Max time') return { min: 1, max: 120, step: 1, disabled, onSave: (value: number) => onSave('steamDuration', value) }
  if (utility.id === 'steam' && label === 'Flow') return { min: 0.1, max: 4, step: 0.1, disabled, onSave: (value: number) => onSave('steamFlow', value) }
  return undefined
}
