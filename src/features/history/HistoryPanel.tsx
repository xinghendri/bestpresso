...
export function HistoryPanel({ shot, status, onOpen }: { shot: PreviousShot | null; status: PreviousShotStatus; onOpen: () => void }) {
  ...
  return <section className="history-section">{shot
    ? <button className="history-card" type="button" onClick={onOpen} aria-label={`Open shot history: ${shot.profileName}`}><div className="history-card__summary metric-scale--medium"><h3>{shot.profileName}</h3><time dateTime={shot.timestamp}>{shotTimestamp(shot.timestamp)}</time><div>{!isCleaning && <span><small>Total yield</small>{shot.totalYield}{shot.totalYield !== '—' && <i>g</i>}</span>}<span><small>Total time</small>{shot.totalTime}{shot.totalTime !== '—' && <i>s</i>}</span></div></div><MiniShotChart shot={shot} /></button>
