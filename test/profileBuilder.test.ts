...
test('editor follows the designed high-level hierarchy without prototype-only fields', () => {
  assert.match(screen, /Moves on when any enabled condition is reached/)
  assert.match(screen, /label: 'Yield'/)
  assert.match(screen, /<span>Yield <img/)
  assert.match(screen, /Stage move-on weight/)
...
test('the header exposes every Decaid profile-level prerequisite and hidden limiter range', () => {
  ...
  assert.match(screen, />Fallback stop volume</)
...
test('stage cards use the compact Figma dimensions and compact further on short screens', () => {
  ...
  assert.match(styles, /\.profile-builder-screen\.pb-screen\{--pb-stage-height:241px\}/)
  ...
  assert.match(styles, /@media\(max-height:650px\)\{\.profile-builder-screen\.pb-screen\{--pb-stage-height:221px\}/)
  assert.match(styles, /\.pb-stage__target-panel\{display:grid;grid-template-columns:minmax\(0,1fr\) 153px/)
  assert.match(styles, /\.pb-stage__conditions-panel\{display:grid;grid-template-columns:1fr 1fr/)
})
...
test('stage sequence remains explicit while one fluid card is edited in place', () => {
  ...
  assert.match(styles, /\.pb-stage__summary-metrics\{height:111px;display:grid;grid-template-columns:1fr 1fr/)
