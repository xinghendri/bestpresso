...
test('stage sequence remains explicit while one fluid card is edited in place', () => {
  ...
  assert.match(styles, /\.profile-builder-screen \.pb-stepper>button\{width:30px;height:30px;aspect-ratio:1;[^}]*flex:0 0 30px;[^}]*border-radius:50%/)
  assert.match(styles, /\.profile-builder-screen \.pb-stepper>\.pb-stepper__value\{min-width:80px;height:40px;min-height:40px;[^}]*border-radius:20px/)
  assert.match(screen, /className="pb-stepper__glyph" aria-hidden="true">−<\/span>/)
  assert.match(screen, /className="pb-stepper__glyph" aria-hidden="true">\+<\/span>/)
