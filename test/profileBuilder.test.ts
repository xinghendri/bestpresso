...
test('editor follows the designed high-level hierarchy without prototype-only fields', () => {
  assert.match(screen, /pb-stage__conditions-rule"><img src=\{skipNext\} alt="" \/><span>Moves on when any enabled condition is reached<\/span><\/p>/)
  ...
  assert.match(screen, /\{comparisonLabel\} &lt;<\/button>/)
  assert.match(styles, /\.pb-stage__conditions-panel\{[^}]*grid-template-columns:repeat\(2,minmax\(0,1fr\)\) 96px[^}]*grid-template-rows:repeat\(2,minmax\(0,1fr\)\)/)
  assert.match(styles, /\.pb-stage__conditions-panel\{[^}]*gap:12px 18px/)
...
test('stage cards use the compact Figma dimensions and compact further on short screens', () => {
  ...
  assert.match(styles, /\.pb-stage__conditions-panel\{display:grid;grid-template-columns:repeat\(2,minmax\(0,1fr\)\) 96px/)
