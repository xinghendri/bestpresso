...
test('revealing the clock repeatedly preserves normal brightness and late reveals cannot undo wake', async () => {
  const { policy, writes } = fixture(60)
  await policy.dim(7)
  await policy.deepen(0)
  await policy.deepen(7)
  await policy.deepen(0)
  await policy.deepen(7)
  await policy.restore()
  await policy.deepen(7)
  assert.deepEqual(writes, [7, 0, 7, 0, 7, 60])
})
