...
test('home insight title stays compact and tablet bars retain top breathing room', () => {
  ...
  assert.match(css, /\.ins-entry-content \{[^}]*padding:24px 20px 20px/)
  assert.match(tabletCss, /\.app-shell \.ins-entry-content \{padding:20px 16px 16px/)
  assert.match(tabletCss, /\.app-shell \.ins-entry-week \{min-height:22px;max-height:66px/)
  assert.match(tabletCss, /\.app-shell \.ins-entry-shot-caption \{padding:6px 16px 16px/)
  assert.match(css, /\.ins-entry-shot-caption \{[^}]*flex-shrink:0/)
  assert.match(css, /\.ins-entry-shot-chart \{[^}]*min-height:0;[^}]*padding:12px 12px 4px/)
})
