...
When Decaid reports the machine in its `espresso` state, Bestpresso automatically replaces the dashboard with the zoomed brewing view. Its native SVG chart plots live pressure, flow, mix/group temperature, and connected-scale weight, together with dashed pressure and flow targets. Samples are synchronized from Decaid's machine and scale snapshot streams and kept in a bounded in-memory buffer; the dashboard returns as soon as the espresso state ends or the machine disconnects.
