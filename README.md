...
The dashboard reads Streamline's five shared favorite-profile slots from Decaid and keeps their saved order; missing or invalid slots are filled from the visible profile list so the home carousel always has five favorites. `Manage profiles` can apply any Decaid profile to the active workflow and can replace or swap each of the five shared favorite slots. Selecting a non-favorite adds it as a sixth carousel item while still rendering only five cards at once, leaving one card hidden until the user scrolls. That ad-hoc profile remains in rotation through its espresso pull; selecting any favorite removes the ad-hoc item and returns the carousel to five. Each profile card builds its green pressure and blue water-flow curves from that profile's ordered pump steps, durations, and transitions, so selecting profiles changes the expected target graph as well as the active Decaid workflow. Fast transitions render as steps, while smooth and eased transitions render as ramps. Across Bestpresso's profile, live, and history graphs, pressure is green, water flow is blue, and weight is magenta.
